...
window = Tk()
window.title('Лабораторна робота 2')
...
class Application:
    ...
    def get_entries(self):
        ...
        P_dims = np.array([int(self.p1_spinbox.get()), int(self.p2_spinbox.get()), int(self.p3_spinbox.get())])
        return samples_filename, output_filename, P_dims, self.polynomial_var.get(), self.weight.get(), self.lambdas.get()
    ...
    def calculate(self):
        ...
        for i in range(model.Y_dim):
            ...
            ax1[i // 2, i % 2].plot(model.results_normed[:, i], label=f"F_{i + 1}")
            ax1[i // 2, i % 2].set_title(f'Ф{i + 1}')
            ax1[i // 2, i % 2].legend(loc=2)

            ax2[i // 2, i % 2].plot(model.Y_array[:, i], label=f"Y_{i + 1}")
            ax2[i // 2, i % 2].plot(model.results[:, i], label=f"F_{i + 1}")
            ax2[i // 2, i % 2].set_title(f'Ф{i + 1}')
            ax2[i // 2, i % 2].legend(loc=2)
