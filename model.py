...
from tools import (
    ...
    Polynom,
    Weight,
    chebyshev_bias_polynomial,
    chebyshev_bias_value,
...
class Model:
    ...
    def __init__(self, s_f, o_f, P_dims, polynomial, weight, lambdas):
        ...
        self.X_normed_df = (self.X_df - self.X_df.min()) / (
            self.X_df.max() - self.X_df.min()
        )
        self.Y_normed_df = (self.Y_df - self.Y_df.min()) / (
            self.Y_df.max() - self.Y_df.min()
        )
        ...
        self.P_dims = P_dims
        self.P1_dim, self.P2_dim, self.P3_dim = [
            self.P_dims[i] for i in range(len(P_dims))
        ]
    ...
    def get_lambdas_generalized(self, b_q):
        ...
        if self.lambda_method == Lambda.TRIPLE_SET.name:
            ...
            F2 = self.F[
                :,
                self.X1_dim * (self.P1_dim + 1) : self.X1_dim * (self.P1_dim + 1)
                + self.X2_dim * (self.P2_dim + 1),
            ]
            F3 = self.F[
                :,
                self.X1_dim * (self.P1_dim + 1)
                + self.X2_dim * (self.P2_dim + 1) : self.X1_dim * (self.P1_dim + 1)
                + self.X2_dim * (self.P2_dim + 1)
                + self.X3_dim * (self.P3_dim + 1),
            ]

            self.lambda01 = np.reshape(
                self.solve_method(F1, b_q), (self.X1_dim, self.P1_dim + 1)
            )
            self.lambda02 = np.reshape(
                self.solve_method(F2, b_q), (self.X2_dim, self.P2_dim + 1)
            )
            self.lambda03 = np.reshape(
                self.solve_method(F3, b_q), (self.X3_dim, self.P3_dim + 1)
            )
        else:
            lambda0 = self.solve_method(self.F, b_q)
            for i in range(self.X1_dim):
                self.lambda01[i] = lambda0[
                    i * (self.P1_dim + 1) : (i + 1) * (self.P2_dim + 1)
                ].T
            for i in range(self.X2_dim):
                self.lambda02[i] = lambda0[
                    i * (self.P2_dim + 1)
                    + self.X1_dim * (self.P1_dim + 1) : (i + 1) * (self.P2_dim + 1)
                    + self.X1_dim * (self.P1_dim + 1)
                ].T
            for i in range(self.X3_dim):
                self.lambda03[i] = lambda0[
                    i * (self.P3_dim + 1)
                    + self.X2_dim * (self.P2_dim + 1)
                    + self.X2_dim * (self.P2_dim + 1) : (i + 1) * (self.P3_dim + 1)
                    + self.X1_dim * (self.P1_dim + 1)
                    + self.X2_dim * (self.P2_dim + 1)
                ].T
            ...
            for j in range(np.sum(self.X_dims)):
                if j < self.X1_dim:
                    self.psi[q, j] = (
                        self.lambda01[j]
                        @ self.F[
                            q, j * (self.P1_dim + 1) : (j + 1) * (self.P2_dim + 1)
                        ].T
                    )
    ...
    def get_a(self):
        ...
        for f in range(self.Y_dim):
            a01 = self.solve_method(
                self.psi[:, : self.X1_dim], self.Y_normed_array[:, f].reshape(-1, 1)
            )

            a02 = self.solve_method(
                self.psi[:, self.X1_dim : self.X1_dim + self.X2_dim],
                self.Y_normed_array[:, f].reshape(-1, 1),
            )

            a03 = self.solve_method(
                self.psi[:, self.X1_dim + self.X2_dim :],
                self.Y_normed_array[:, f].reshape(-1, 1),
            )
    ...
    def get_c(self):
        ...
        for i in range(self.Y_dim):
            ...
            for q in range(self.Q):
                Fij[q, 0] = self.a0[i, : self.X1_dim].reshape(1, -1) @ self.psi[
                    q, : self.X1_dim
                ].reshape(-1, 1)
                Fij[q, 1] = self.a0[i, self.X1_dim : self.X1_dim + self.X2_dim].reshape(
                    1, -1
                ) @ self.psi[q, self.X1_dim : self.X1_dim + self.X2_dim].reshape(-1, 1)
                Fij[q, 2] = self.a0[i, self.X1_dim + self.X2_dim :].reshape(
                    1, -1
                ) @ self.psi[q, self.X1_dim + self.X2_dim :].reshape(-1, 1)
            self.c0[i] = self.solve_method(
                Fij, self.Y_normed_array[:, i].reshape(-1, 1)
            ).T
    ...
    def get_restored_functions(self):
        ...
        for f in range(self.Y_dim):
            ...
            for i in range(self.X3_dim):
                a_lambda03[i] = (
                    self.a0[f, i + self.X1_dim + self.X2_dim] * self.lambda03[i]
                )
            ...
            polynomials[f] = np.concatenate(
                (c_va_lambda01[f], c_va_lambda02[f], c_va_lambda03[f])
            )
        return polynomials
        ...
        for q in range(self.Q):
            for f in range(self.Y_dim):
                for j in range(self.X1_dim):
                    for k in range(self.P1_dim + 1):
                        results_normed[q, f] += (
                            self.polynomials[f, (self.P1_dim + 1) * j + k]
                            * self.F[q, (self.P1_dim + 1) * j + k]
                        )
                shift = self.X1_dim * (self.P1_dim + 1)
                for j in range(self.X2_dim):
                    for k in range(self.P2_dim + 1):
                        results_normed[q, f] += (
                            self.polynomials[f, (self.P2_dim + 1) * j + k + shift]
                            * self.F[q, (self.P2_dim + 1) * j + k + shift]
                        )
                shift += self.X2_dim * (self.P2_dim + 1)
                for j in range(self.X3_dim):
                    for k in range(self.P3_dim + 1):
                        results_normed[q, f] += (
                            self.polynomials[f, (self.P3_dim + 1) * j + k + shift]
                            * self.F[q, (self.P3_dim + 1) * j + k + shift]
                        )
    ...
    def get_functions(self):
        ...
        self.results = np.array(
            [
                [
                    self.results_normed[q][i] * (maxs[i] - mins[i]) + mins[i]
                    for i in range(self.Y_dim)
                ]
                for q in range(self.Q)
            ]
    ...
    def calculate(self):
        self.get_F()
        self.polynomials = np.zeros(
            (self.Y_dim, np.sum(self.X_dims * (self.P_dims + 1)))
        )
        ...
        self.get_functions()
        self.residuals = [
            f"{np.max(np.abs(self.Y_normed_array[:, i] - self.results_normed[:, i])):.6f}"
            for i in range(len(self.results_normed[0]))
        ]
